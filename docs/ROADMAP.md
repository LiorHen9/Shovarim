...
## Phase 10 — מועדוני חברות והטבות
המשתמש מצהיר על כרטיסי המועדון שברשותו, והאפליקציה מציגה לו את ההטבות הרלוונטיות ומאפשרת לשוחח עליהן עם הבוט. הקטלוג דו-שכבתי — מועדון (מפעל הפיס) ותחתיו כרטיסים (רגיל, VIP) — כי ההטבות נבדלות בין הדרגים, וזו ההבחנה שבגללה הפיצ'ר קיים. ארבע שכבות: (1) קטלוג + סימון עצמי + UI, (2) שליפת ההטבות מהרשת, (3) הצגתן פר-משתמש, (4) חשיפה לבוט. ראו `docs/DECISIONS.md` ADR #61.

> ⚠️ שכבה 2 חסומה על החלטה פתוחה: allowlist דומיינים מוכר מול fetch כללי מוקשח (חסימת IP פרטי מפני SSRF, timeout, הגבלת גודל תגובה) — `docs/ISSUES_SPRINT.md` issue #50. **להכריע כ-ADR לפני שורת קוד אחת של שליפה.** שכבה 1 אינה מבצעת אף בקשת HTTP יוצאת.

### שלב 10.1 — קטלוג, חברויות ו-UI ✅ הושלם (2026-09-06)
שלושה collections חדשים: `clubs` ו-`clubCards` (קטלוג סגור, `allow write: if false`, נזרע ב-`scripts/seed-clubs.ts` / `npm run seed:clubs`) ו-`clubMemberships` עם מזהה דטרמיניסטי `${uid}_${clubCardId}` שנאכף ב-`firestore.rules` — מה שהופך כפילות לבלתי-ניתנת-לייצוג ומאפשר סימון/ביטול ב-`setDoc`/`deleteDoc` עיוור בלי read-then-write. המועדון נגזר מהקטלוג ואינו מדונרמל לחברות, כי שדה שהלקוח כותב הוא שדה שהלקוח יכול לזייף. עמוד `/clubs` (`src/components/clubs/ClubsGrid.tsx`, `src/hooks/useClubCatalog.ts`, `useClubMemberships.ts`), רשום ב-`src/proxy.ts` וב-`NAV_LINKS`; רכיב `ui/checkbox` נוסף (shadcn). כל מועדון הוא `fieldset`/`legend` — ה-checkbox מתויג בשם הדרג בלבד ("VIP"), וה-legend הוא מה שנותן לקורא-מסך את ההקשר. נכלל ב-`buildUserDataExport` (עם שמות בטקסט מלא, לא slugs) וב-`functions/src/accountDeletion.ts`.
- **ממצא 1 — ה-checkbox לא הגיב עד חזרת הכתיבה**: הוא היה controlled ישירות מול ה-`onSnapshot`, כך שעל חיבור איטי המשתמש לוחץ ולא קורה כלום. תוקן ל-override אופטימי שחי רק לאורך הכתיבה, מתגלגל אחורה בכישלון, ומשבית את אותה תיבה בזמן השמירה (גם מונע מרוץ tick/untick מהיר ששתי כתיבותיו קובעות תוצאה לפי סדר ההגעה). ה-E2E חשף את זה (`locator.check: Clicking the checkbox did not change its state`) והוא גם מה שהאיץ מ-18ש׳ ל-1.6ש׳.
- **ממצא 2 — באג ותיק ב-`tests/e2e/helpers/auth.ts`, שגרם ל-flakiness בכל הסוויטה**: `grantConsent()` היא כתיבת Firestore מהלקוח, והדיאלוג נסגר על ה-snapshot המקומי (latency compensation) — לפני ה-ack מהשרת. ה-helper חזר באותו רגע, הטסט ניווט מיד, `page.goto` הרס את ה-context והשליך את הכתיבה התלויה, ולכן העמוד המוגן הבא קרא מהשרת, לא מצא הסכמה, והקפיץ את המודאל **באמצע הטסט** — שם ה-overlay בולע כל קליק בשקט. זה מה שהכשיל לסירוגין את `listInvite` ואת `settings` הרבה לפני Phase 10. תוקן: ה-helper מרענן ומאמת שהדיאלוג לא חוזר בטעינה נקייה, כלומר שההסכמה באמת הגיעה לשרת. הסוויטה המלאה עברה מ-57 עוברים + 3 flaky ל-**60 עוברים, אפס flaky**.
- אימות: `typecheck`/`lint`/`build` נקיים, `/clubs` נבנה כ-`ƒ (Dynamic)`; `npm --prefix functions run build` נקי; `test:unit — 88`; `test:rules — 71` (11 חדשים, כולל שני מקרי ה-doc-id שמונעים כפילות); `test:e2e` — 3 טסטים חדשים ב-`clubs.spec.ts` + `/clubs` נוסף לסריקת axe ולטסט הכותרות ב-`accessibility.spec.ts`. הקטלוג נזרע ל-E2E דרך `/e2e/seed-clubs` + `src/actions/testSeed.ts`, מוגן-אמולטור באותה צורה כמו `mintTestCustomToken` (ADR #18), עם fixture ולא עם הקטלוג האמיתי כדי שהטסט לא יישבר כשמוסיפים מועדון.

### שלב 10.1.a — מילוי הקטלוג ולוגואים ✅ הושלם (2026-09-06)
הקטלוג עבר משני מועדוני-דוגמה לשבעה אמיתיים: מפעל הפיס (רגיל + VIP), חבר, שווה, בהצדעה, הוט, טוב+, אשמורת. הנתונים הועברו ל-`scripts/clubs.data.ts` — מודול נפרד מהסקריפט, כדי שטסט יוכל לייבא אותו בלי להריץ זריעה. כל מועדון קיבל לוגו כקובץ מקומי ב-`public/clubs/` (ADR #61 החלטה 7); `Club` קיבל `logoUrl: string | null` ו-`website` הפך ל-nullable.
- **שלוש כתובות שהניחוש הסביר עליהן שגוי**, וכולן נבדקו מול המקור במקום להישען על תבנית: חבר הוא `hvr.co.il` ולא `hever`; בהצדעה הוא `behatsdaa.org.il` ולא `behatzdaa` (הערך השגוי כבר היה בקטלוג מ-10.1 — פרודקשן מעולם לא נזרע, ולכן לא היה נזק חי); ותוכנית ההטבות של מפעל הפיס יושבת ב-`paisplus.co.il`, לא באתר של מפעל הפיס. לאשמורת אין אתר חי בכלל (`itu.org.il` מושהה, `ashmoret.co.il` מפנה לעמוד ריק) — ומכאן `website: null`.
- **חוב ידוע**: פיס פלוס מפרסם בפועל ארבעה דרגי זכאות (כסף / זהב / זהב+ / פלטינום) ושומר את הטבות ה-"VIP+" לשניים העליונים. רגיל/VIP הוא פישוט שנבחר ביודעין; שכבה 10.2 תצטרך להרחיב את הרשימה ולמפות חברויות קיימות (ADR #61 החלטה 8).
- **`tests/unit/clubCatalog.test.ts` (5 טסטים חדשים, 88 → 93)** מאמת שכל `logoUrl` מצביע על קובץ שקיים תחת `public/`, שה-ids ייחודיים, שלכל מועדון יש לפחות כרטיס אחד, ושכל `website` הוא https או null. הצורך אמיתי: ה-E2E רץ במכוון מול fixture ולא מול הקטלוג האמיתי, ולכן נתיב לוגו שגוי היה עובר typecheck, lint, build ו-E2E ומתגלה רק כתמונה שבורה בפרודקשן.
- **`clubs.spec.ts` קיבל טסט רביעי** שמכסה את ארבעת המצבים של לוגו/קישור בטעינה אחת — ה-fixture מחזיק מועדון אחד עם שניהם ואחד בלי אף אחד. הבדיקה על התמונה היא `naturalWidth > 0` ולא על קיום ה-`<img>`, כי `<img>` עם נתיב שבור עדיין קיים ב-DOM.
- אימות ויזואלי: צילום `/clubs` בשני ה-themes. האריח הלבן הקבוע מאחורי הלוגו הוא מה שמונע משלושת הלוגואים על רקע לבן (מפעל הפיס, שווה, הוט) להיראות כריבועים חיוורים ב-dark mode.
...
### שלב 10.2 — סקרייפרים להטבות ✅ הושלם (2026-09-07)
ארבעה מועדונים נסרקים אוטומטית כל יום ב-04:00 (`scrapeClubBenefits`, gen2 `onSchedule`, `Asia/Jerusalem`), וההטבות נשמרות ב-`clubBenefits` כ**תמונת מצב מלאה** — מה שהמועדון הפסיק להציע נמחק ולא נשאר כשארית. ראו `docs/DECISIONS.md` ADR #62.
- **ה-ADR שחסם את השלב נכתב ראשון.** issue #50 השאיר פתוחה את ההכרעה allowlist מול fetch כללי מוקשח. ההכרעה: **allowlist קשיח של hosts** ב-`functions/src/benefits/http.ts`, בנימוק שאין בפיצ'ר הזה אף URL שמגיע ממשתמש — ולכן ה-SSRF אינו מוגן אלא בלתי-נגיש, וה-allowlist הוא מה שמשמר את זה כשהקוד יגדל.
- **חמישה משבעת המועדונים רצים על שתי פלטפורמות white-label משותפות**, ולכן ארבעת הנתמכים מכוסים ב-**שלושה** פרסרים: הוט (`api.hot.co.il`, JSON ציבורי, 7,101 הטבות), טוב+ ופיס פלוס (אותה פלטפורמת Dolce — `window.__PRELOADED_STATE__` מוטמע בכל `/category/{id}`), וחבר (שלושה קבצי JSON סטטיים).
- **שווה ובהצדעה נשללו בבדיקה ולא בהנחה**: נחלצו כל 64 נתיבי ה-API של פלטפורמת DTS מה-bundle של `hatavot.shavve.co.il`, ותשעת נתיבי הקטלוג נבדקו מול שני ה-hosts — **כולם 401**, כלומר middleware גלובלי ולא הרשאה נקודתית. הם רשומים ב-`registry.ts` עם `adapter: null` וסיבה שהפאנל מציג, כי מועדון שנעדר מהרשימה נראה בדיוק כמו מועדון שהסקרייפר שלו נשבר.
- **הדריסה מוגנת בשסתום.** ריצה מסרבת לכתוב או למחוק אם לא חזרה אף הטבה, או אם המקור הצטמק מתחת ל-50% מהריצה המוצלחת האחרונה — סטטוס `aborted`, והנתונים הקיימים נשארים. ההשוואה על `sourceTotal` ולא על `written`, אחרת כל שינוי תקרה היה מפיל את הריצה הבאה.
- **תקרת חילוץ מהפאנל, ברמת מועדון וברמת כרטיס, ברירת מחדל 50** — בטפסים הקיימים ב-`/admin/clubs`, כי התקרה היא תכונה של המועדון. `0` = ללא הגבלה. התקרה חוסמת גם את המשיכה עצמה ולא רק את האחסון: הוט מושכת 2 עמודים במקום 237.
- **חבר: מדריך בתי עסק, לא הטבות מתומחרות.** הקבצים הפתוחים שלה מכילים 345 עסקים ו-1,596 סניפים בלי מחיר כלל. לכן נוסף `benefitKind: "offer" | "acceptance"` — בלעדיו כ-1,900 שורות חסרות מחיר היו נקראות כהטבות שהפרסור שלהן נכשל. הקטלוג גם גדל: לחבר יש שני מוצרי כרטיס, ונוסף `hever-teamim` (המזהה הקיים `hever-regular` נשמר כדי לא לייתם חברויות).
- **באג שנתפס בהרצה חיה ולא בטסט**: הגרסה הראשונה של פרסר Dolce עשתה BFS על קטגוריות אבל הציעה את כל מוצרי הקטגוריה בבת אחת — ועמוד קטגוריה מחזיר עד 1,000 מוצרים, כך שהקטגוריה הראשונה מילאה את התקרה לבדה ו-50 מתוך 50 ההטבות של טוב+ נשמרו תחת "חגי תשרי". התיקון: מכסה של 8 לקטגוריה במעבר ראשון, מעבר שני round-robin ללא בקשות נוספות, ודחיית קטגוריית דף-הבית לסוף לפי הדגל `is_homepage`. אחרי התיקון — שבע קטגוריות לכל מועדון.
- **`/admin/benefits`**: סטטוס לכל מועדון (מתי רץ, `written` מול `sourceTotal`, כמה שאריות נמחקו, איזו תקרה קטעה, שגיאות) וכפתור "הרץ עכשיו" דרך callable `adminScrapeBenefitsNow`. קריאה בלבד במכוון — הריצה הבאה תדרוס כל עריכה.
- אימות: `typecheck`/`lint`/`build`/`functions build` נקיים; `test:unit` 124 → **202**; `test:rules` 71 → **77**; `test:e2e` 66 → **69**; והרצה חיה מלאה מול האמולטור וארבעת המקורות האמיתיים, כולל אימות הדריסה (תקרה 50 → 10 = 10 נשמרו, 40 נמחקו).

### שלב 10.3 — הצגת ההטבות פר-משתמש

### שלב 10.4 — כלי MCP לבוט
