...
**עדכון (2026-09-02, ראו ADR #48)**: אחרי שה-PR הזה נפרס, `/admin/users/[uid]` עבור `liorh@hms.co.il` המשיך לקרוס — עם אותו digest בדיוק. התברר, מול Firebase Console, שחשבון ה-Auth שלו מעולם לא נמחק בפועל (שלוש נקודות הכישלון של ADR #46 קרו כולן *לפני* שורת ה-`auth.deleteUser` ב-`deleteUserAccount()`) — כך שהתרחיש שה-ADR הזה טיפל בו (`authAccountExists: false`) לא היה הסיבה האמיתית לקריסה בפועל. התיקון כאן עדיין נכון ושמור (התיעוד "idempotent-safe" צריך להיות אמיתי, לא רק כתוב), אבל הוא לא זה שפתר את התקלה שדווחה. הסיבה האמיתית ב-ADR #48.

## 48. Firestore `Timestamp` לא ניתן להעביר כ-prop משרת ל-Client Component — `deletionRequestedAt` ב-`UserDeletionSection`
**תאריך**: 2026-09-02

**רקע**: אחרי ADR #47, `/admin/users/[uid]` עבור `liorh@hms.co.il` המשיך לקרוס באותו digest. הלוגים בפועל (`resource.type="cloud_run_revision"`, `logName=".../logs/run.googleapis.com%2Fstderr"`, מתואם ל-trace ID של הבקשה) הראו את השגיאה האמיתית: `Error: Only plain objects, and a few built-ins, can be passed to Client Components from Server Components. Classes or null prototypes are not supported.` — שגיאה סטנדרטית של Next.js App Router, לא קשורה כלל ל-Auth.

**הסיבה**: `page.tsx` (Server Component) העביר את `profile.deletionRequestedAt` — מופע מחלקה אמיתי (`Timestamp` מ-`firebase-admin/firestore`, לא אובייקט רגיל) — ישירות כ-prop ל-`UserDeletionSection`, שהוא `"use client"`. React Server Components אוסר סריאליזציה של מופעי class בגבול הזה. הבאג היה קיים כבר משלב 9.4 (ADR #45) אבל מעולם לא התבטא בפרודקשן, כי אף משתמש שנצפה בפאנל עד עכשיו לא היה עם `deletionRequestedAt` שאינו `null` בפועל — `liorh@hms.co.il` הוא ככל הנראה המשתמש הראשון עם ערך אמיתי בשדה הזה שנצפה בעמוד הזה. שאר הקוד בפרויקט כבר מכיר את הכלל הזה ופותר אותו נכון: `channelLinks.ts`'s `toSummary()` וגם `src/actions/privacy.ts` כבר ממירים `Timestamp` ל-ISO string (`.toDate().toISOString()`) לפני שהוא חוצה לכיוון client — הנקודה הזו בשלב 9.4 פשוט פספסה את התבנית הקיימת.

**החלטה — המרה ל-ISO string בעמוד, לפני שהערך חוצה לקומפוננטת client, לא שינוי בטיפוס המשותף `UserProfile`.** `page.tsx` ממיר: `profile.deletionRequestedAt ? profile.deletionRequestedAt.toDate().toISOString() : null`. `UserDeletionSection` מקבל `deletionRequestedAt: string | null` (במקום `UserProfile["deletionRequestedAt"]`), ובונה `Date` מקומי (`new Date(deletionRequestedAt)`) לפני קריאה ל-`getDeletionEligibleAt`. `UserProfile.deletionRequestedAt: Timestamp | null` עצמו נשאר ללא שינוי — הוא משמש נכון גם היום בקומפוננטות client כמו `DeleteAccountSection`/`DeletionPendingBanner`, ששם הערך נוצר ונצרך כולו בצד לקוח (Client SDK Timestamp, ללא מעבר גבול RSC) ולכן לא נזקק לאותה המרה.

**למה זה לא נתפס קודם**: אין E2E/בדיקה ידנית שכיסתה את `/admin/users/[uid]` עבור משתמש עם `deletionRequestedAt` לא-`null` בפועל — Phase 9.4 נבדק מול משתמשי בדיקה "רגילים" בלבד. לקח: כשמוסיפים prop חדש מ-Server Component ל-`"use client"`, לבדוק אם הטיפוס שלו הוא Firestore `Timestamp` (או כל class אחר) ולא רק "זה עובד ב-TypeScript" — `tsc`/ESLint לא תופסים את חוק ה-serialization של RSC בזמן build.

## 49. פאנל ניהול — מעקב שימוש/עלות Claude API: `claudeUsageLog` פר-קריאה, כתיבה לא-חוסמת, ללא הישענות על ה-Admin API של Anthropic (Phase 9.5)
**תאריך**: 2026-09-02

**רקע**: `src/lib/mcp/agentLoop.ts` לא אסף מעולם את `response.usage` מקריאות `messages.create()`. לפני המימוש נבדקה השאלה "אולי Anthropic כבר פותרת את זה" — נטען ה-skill `claude-api` (כפי שה-ROADMAP דרש במפורש) ונשלף בפועל (WebFetch) התיעוד הרשמי של ה-Usage & Cost Admin API (`https://platform.claude.com/docs/en/manage-claude/usage-cost-api.md`).

**ממצא מכריע לפני שנכתבה שורת קוד — Anthropic לא רואה את ה-`uid` שלנו בכלל.** ממדי הסינון/הקיבוץ של `usage_report`/`cost_report` הם `api_key_id`/`workspace_id`/`model`/`service_tier`/`context_window`/`inference_geo`/`speed` — אין שום שדה `metadata`/תג מותאם על בקשת ה-Messages API עצמה שמגיע לדוחות האלה. ה-FAQ הרשמי שם עונה במפורש: "How do I get per-user cost breakdowns?" → "For general API usage with many keys, use the Usage API to track token consumption as a cost proxy" — כלומר הפתרון היחיד שהם מציעים הוא API key אחד per user, עם אזהרה מפורשת על "performance limitations" בקנה מידה. גם זה לא רלוונטי לפרויקט: הגישה ל-Claude היא דרך WIF (service account אחד, ADR #20) ולא API keys, כך שאין אפילו את ה"פרוקסי" הגרוע הזה לנצל. **מסקנה: אין קיצור דרך — תיעוד עצמי הוא הדרך היחידה לפילוח פר-`uid`.**

**החלטה (1) — רשומה אחת לכל קריאת `messages.create()`, לא לכל הודעת משתמש, מנקודת כתיבה משותפת יחידה.** `logClaudeUsage` (`src/lib/mcp/claudeUsageLog.ts`) נקראת מתוך `runAgentTurn` עצמו (`agentLoop.ts`), לא משוכפלת בשלושת/ארבעת קוראיו (`channelChat.ts`/`route.ts`/`mcp-cli.ts`/`run-chat-scenario.ts`) — אותו עיקרון בדיוק כמו `withToolExecution` ל-audit log של tool calls (ADR #22). סבב עם tool calls מבצע כמה קריאות מודל באותה לולאה, וכל אחת מחויבת בנפרד — צריך את זה כדי לחשב עלות אמיתית, לא רק "עלות ההודעה האחרונה בסבב".

**החלטה (2) — הכתיבה מופעלת (לא `await`-ת) מיד אחרי כל תשובת מודל, ונאספת ב-`Promise.all` שרץ ב-`finally` לפני שהפונקציה חוזרת בכל נתיב (כולל שגיאה).** זו הייתה השאלה המרכזית לפני המימוש: "האם זה יפגע ב-latency של השיחה?". התשובה שהתגבשה בדיון: כתיבת מסמך בודד ל-Firestore (`europe-west4`, אותו region כמו App Hosting) היא ~20-60ms — שולית לעומת קריאת Claude עצמה (שניות) — וכבר יש בדיוק את אותו סוג תוספת בנתיב הזה (`writeAuditLog` בתוך `withToolExecution`, מ-ADR #21/#22). אבל הכתיבה **כן** הייתה עלולה לעכב טקסט שכבר מוכן להישלח למשתמש: ב-`POST /api/chat`, `onText` שולח כל בלוק טקסט ל-NDJSON stream ברגע שהוא זמין (אין streaming טוקנים מ-Claude עצמו — הקריאה non-streaming — אבל יש streaming של הבלוקים המוכנים). לכן הכתיבה **לא** ממתינה לפני `onText`/המשך הלולאה — היא רק מופעלת, וכל הכתיבות הממתינות נאספות ו-`await`-ות ביחד ב-`finally`, אחרי שהתשובה כבר בדרך למשתמש. ה-`finally` (ולא רק אחרי ה-`return` המוצלח) הוא הכרחי כדי שקורא serverless (Cloud Function של webhook הוואטסאפ) לא ישלח את תשובתו ויקפיא את התהליך לפני שהכתיבה נחתמה.

**החלטה (3) — הכתיבה לעולם לא זורקת, בניגוד ל-`writeAuditLog` הקיים.** `logClaudeUsage` בולעת שגיאות (`console.error` בלבד). זהו לדג'ר חשבונאי, לא לדג'ר אבטחה/ציות (`auditLog`) — הפסקת שיחה עם משתמש בגלל תקלת רישום עלות היא מחיר גבוה יותר מרשומת עלות חסרה. הוחלט במפורש **לא** לאחד את שני מנגנוני ה-log (למשל דרך `writeAuditLog` המשותף) בגלל ההבדל הזה בסמנטיקת השגיאה.

**החלטה (4) — `firestore.rules`: `allow read, write: if false` לחלוטין, כולל לבעלים — בשונה מ-`auditLog` (`allow read: if isExistingOwner()`).** נשקל לתת לבעלים read ישיר בדפוס הזהה ל-`auditLog`, ונדחה: אין כרגע פיצ'ר משתמש שצריך לראות את זה (בניגוד ל-`auditLog`, שמיוצא כחלק מ-Right to Access, Phase 4.1), ותצוגת האדמין עוברת Admin SDK ממילא — פתיחת read לבעלים הייתה מוסיפה משטח בלי צורך מיידי.

**החלטה (5) — `estimatedCostUsd` הוא הערכה סטטית (`src/lib/mcp/pricing.ts`), לא נגזרת מה-Admin API.** טבלת תמחור קשיחה ל-`MODEL_ID` הנוכחי (`claude-sonnet-5`: $2/$10 ל-1M input/output, מאומת מול טבלת התמחור העדכנית של ה-skill `claude-api`) + מכפילי cache read/write מתועדים (~0.1x/~1.25x ממחיר ה-input, קירוב לא-רשמי-פר-מודל). שדות הטוקנים הגולמיים נשמרים גם הם — הם מקור האמת, `estimatedCostUsd` הוא תמונת מצב שניתן לחשב מחדש. נופלת בחזרה ל-0 עם `console.error` על מודל לא מוכר בטבלה, לא זורקת — אותו עיקרון כמו החלטה 3.

**החלטה (6) — תצוגת אדמין: הרחבת הכרטיס הקיים "שימוש" ב-`/admin/users/[uid]`, לא עמוד/section נפרד.** `getClaudeUsageSummaryForUid` (`src/lib/services/adminClaudeUsage.ts`) עושה `count()`+`sum("estimatedCostUsd")` aggregation (לא הבאת כל המסמכים) על `where("uid","==",uid)`, אותו עיקרון בדיוק כמו ספירת כרטיסים/רשימות הקיימת (ADR #43). מתווסף ל-`Promise.all` הקיים ב-`getUserDetail`, לא שאילתה נפרדת שרצה בנתיב אחר.
> **תיקון (2026-09-05)**: הנוסח המקורי של החלטה (6) טען שהאגרגציה הזו היא "שדה בודד, בלי אינדקס מרוכב". זה התברר כשגוי בפרודקשן: `sum()` (בשונה מ-`count()`) חייב לקרוא את השדה המסוכם מתוך האינדקס עצמו, ולכן שילוב `where()` עם `sum()` דורש אינדקס מרוכב. האינדקס `uid ASC, estimatedCostUsd ASC` נוסף ב-commit ba3c0b7 אחרי ש-`/admin/users/[uid]` נפל ב-500 (פוסט-מורטם ב-`docs/DEPLOYMENT.md`).

**נדחה**: (א) הישענות על ה-Admin API של Anthropic לפילוח פר-משתמש — לא אפשרי במבנה הנוכחי, ראו הממצא למעלה; (ב) API key אחד per Shovarim user כדי "לרמות" את ה-Usage API לתת פילוח — נדחה גם כי Anthropic עצמה מתייגת את זה כפתרון עם מגבלות ביצועים בקנה מידה, וגם כי לא תואם את ארכיטקטורת ה-WIF הקיימת (ADR #20); (ג) איחוד עם `writeAuditLog` המשותף — נדחה בגלל הבדל סמנטיקת השגיאה (החלטה 3); (ד) `allow read` לבעלים על `claudeUsageLog` — נדחה בהיעדר פיצ'ר משתמש קיים שצריך אותו (החלטה 4); (ה) `await` סינכרוני של הכתיבה לפני `onText` — נדחה כדי לא לעכב טקסט שכבר מוכן להישלח (החלטה 2).

**אימות**: `typecheck`/`lint`/`build` נקיים. `test:rules` — 60/60 (59 קיימים + 1 חדש: קריאה/כתיבה ל-`claudeUsageLog` נדחית גם לבעלים המאומת). `test:unit` — 72 (67 קיימים + 5 חדשים ל-`estimateCostUsd`: תמחור input/output בסיסי, מכפילי cache write/read, סכימת ארבעת הרכיבים, נפילה ל-0 על מודל לא מוכר, אפס שימוש). אימות מקצה-לקצה אמיתי מול Firestore/Auth emulators + קריאת Claude API אמיתית (סקריפט חד-פעמי, לא נשמר): סבב שיחה יחיד כתב בדיוק רשומת `claudeUsageLog` אחת עם טוקנים תואמים לתשובה שהתקבלה בפועל (כולל `cacheCreationInputTokens` — ה-cache breakpoint על ה-system prompt אכן נצרך), ו-`getClaudeUsageSummaryForUid` החזיר `calls`/`estimatedCostUsd` תואמים.

**מה עוד נשאר**: אנליטיקס בקנה מידה (Phase 9.6) — Firestore aggregations → Firebase Extension ל-BigQuery → GA4, ראו `docs/ROADMAP.md`.

## 50. פאנל ניהול — סקירת צריכת Claude כלל-מערכתית במסך הבית: אגרגציות לפי חלון זמן, תקציב כמשתנה סביבה, ו-degradation פר-כרטיס (Phase 9.6 שכבה 1)
**תאריך**: 2026-09-05

**רקע**: אחרי Phase 9.5 (ADR #49) נתוני העלות היו נגישים רק פר-משתמש, בעמוד `/admin/users/[uid]`. לא הייתה שום תשובה לשאלה "כמה אנחנו מוציאים בסך הכל", ומסך הבית של האדמין (`/admin`) נשאר ה-shell של Phase 9.1 — כותרת ולינק אחד. `docs/ROADMAP.md` Phase 9.6 כבר הגדיר את שכבה (1) כ-"Firestore aggregation queries לטייל-ים בסיסיים בדשבורד — אפס תשתית חדשה"; זה המימוש שלה.

**החלטה (1) — ארבעה חלונות זמן, אותה צורת aggregation שכבר קיימת.** `getClaudeUsageOverview` (`src/lib/services/adminClaudeUsage.ts`, אותו קובץ ולא שירות חדש) מריץ ארבע aggregations ב-`Promise.all`: 24 שעות, 7 ימים, מתחילת חודש הלוח, ומאז ומתמיד. כל אחת היא `count()`+`sum("estimatedCostUsd")` — בדיוק אותה שאילתה כמו `getClaudeUsageSummaryForUid`, רק עם `where("createdAt", ">=", …)` במקום `where("uid","==",…)`. העלות נשארת מספר קבוע וקטן של reads לכל חלון, ללא תלות בגודל הלוג.

**החלטה (2) — גבול החודש נמדד ב-UTC, לא ב-Asia/Jerusalem.** `Date.UTC(y, m, 1)`. Anthropic מחייבת לפי UTC, ו-`estimatedCostUsd` הוא ממילא הערכה שלנו (החלטה 5 ב-ADR #49) — יישור לשעון ישראל היה הופך את 3 השעות הראשונות/אחרונות של כל חודש לדו-משמעיות בלי להוסיף שום דיוק. מתועד ב-UI עצמו (הערה מתחת לכרטיסים) כדי שהמספר לא ייקרא כ"חודש לפי השעון המקומי".

**החלטה (3) — בנק קרדיטים משולם מראש כמשתני סביבה, לא תקציב חודשי ולא מסמך הגדרות ב-Firestore.** שלושה משתנים ב-`apphosting.yaml` (`availability: [RUNTIME]`, נקראים עצלנית בתוך הפונקציה כמו `src/lib/whatsapp/deepLink.ts`, כך ש-`next build` לא נחסם): `CLAUDE_CREDIT_BANK_USD` (סך הקרדיט שנרכש — מכנה הפס), `CLAUDE_CREDIT_BALANCE_USD` ו-`CLAUDE_CREDIT_BALANCE_AT`. נדרשים שלושתם יחד; חסר או לא ניתן לפרסור → הפאנל פשוט לא מוצג, שאר הדף תקין.

- **למה בנק ולא תקציב חודשי**: המודל בפועל הוא קרדיט משולם מראש שלא מתאפס ב-1 לחודש — הוא רק יורד עד לטעינה מחדש. כרטיס "החודש הנוכחי" נשאר כמדד קצב שריפה, אבל הפאנל שמתחתיו עונה על שאלה אחרת: "כמה נשאר". מעבר לתקציב חודשי בעתיד הוא שינוי מקומי בשכבת השירות בלבד.
- **למה יתרה+חותמת זמן ולא סכימה של כל הלוג**: `claudeUsageLog` קיים רק מ-Phase 9.5 והלאה, כך שניצול שקדם לו **בלתי נראה** לנו וסכימת כל הקולקציה מחסירה אותו. נשקל קיזוז בסיס קבוע (`בנק − ניצול_לא_מתועד − סכום_הלוג`) ונדחה משתי סיבות: את הקיזוז אפשר לחשב רק אחרי deploy (צריך לדעת מה סכום הלוג באותו רגע), והסטייה מול החיוב האמיתי רק גדלה עם הזמן בלי מנגנון תיקון. עיגון ליתרה שנקראה בפועל ב-Console פותר את שתיהן: כל מה שלפני החותמת — מתועד או לא — כבר בלוע במספר, ורק רשומות שאחריה נסכמות. קריאה מחדש של היתרה האמיתית ועדכון שני הערכים **מאפסת את הסטייה**, וטעינת קרדיט מחדש היא אותה עריכה בדיוק (בתוספת העלאת `BANK_USD`).
- **מלכודת מתועדת**: עדכון `BALANCE_USD` בלי `BALANCE_AT` (או להפך) סופר פעמיים את ההוצאה שביניהם. שני הערכים חייבים להתעדכן יחד — מתועד ב-`apphosting.yaml`, ב-`.env.example` וב-`docs/DEPLOYMENT.md`, ומוצג גם ב-UI עצמו (הפאנל אומר מאיזה תאריך היתרה נקראה).
- **המסלול דרך Firestore נדחה** מאותה סיבה כמו קודם: `adminSettings/{doc}` עם טופס עריכה היה גורר collection חדש, match block ב-`firestore.rules`, Server Action, טופס ו-validation — הכל כדי לערוך שלושה ערכים שזזים לעיתים רחוקות. המחיר המודע: שינוי מחייב rollout.
- **חשוב**: זו תצוגה בלבד ולא תקרה נאכפת — שום דבר לא נחסם כשהבנק נגמר; מנגנון הבלימה בפועל נשאר `RATE_LIMITS` ב-`src/lib/mcp/config.ts`. אם הבנק מתרוקן, ה-API עצמו יתחיל להחזיר שגיאות והצ'אט ייפול — הפאנל הוא אזהרה מוקדמת, לא הגנה.

**החלטה (4) — כל חלון נתפס ב-`try/catch` בנפרד ומחזיר `null`, במקום `Promise.all` שנופל כולו.** זו תגובה ישירה לתקלה של 2026-09-05 (פוסט-מורטם ב-`docs/DEPLOYMENT.md`): אינדקס מרוכב חסר גרם ל-`FAILED_PRECONDITION` באגרגציית ה-`sum()`, וכיוון שהיא ישבה בתוך `Promise.all` משותף ב-`getUserDetail` היא הפילה את **כל** עמוד `/admin/users/[uid]` ב-500. כאן מדובר במסך הבית של הפאנל, כך שאותה תקלה הייתה מרוקנת את האדמין לגמרי. כרטיס בודד שנכשל מציג "לא זמין"; השאר ממשיכים לעבוד. זו רשת ביטחון ולא תחליף לאינדקס — `createdAt ASC, estimatedCostUsd ASC` נוסף ל-`firestore.indexes.json`, ובדיקת האינדקס בפרודקשן (לא ב-emulator, שבונה אינדקס לכל שאילתה שמגיעה אליו) היא חלק מרשימת האימות.

**החלטה (5) — עמודת עלות פר-שורה בטבלת המשתמשים נדחתה.** נשקלה במפורש ונדחתה: אין ב-Firestore group-by, כך שעלות לכל שורה משמעותה aggregation נפרדת לכל משתמש — 25 שאילתות נוספות בכל טעינה של `/admin/users`, כדי להציג מידע שכבר קיים במרחק קליק אחד בעמוד הפרטים. מאותה סיבה נדחתה גם רשימת "טופ-N משתמשים יקרים" בשלב הזה: היא מחייבת סריקה מלאה של `claudeUsageLog` (read לכל מסמך) או טבלת rollup, וזה בדיוק הטריגר למעבר לשכבה (2) של Phase 9.6 — ה-Extension ל-BigQuery.
...
## 57. בר נגישות והצהרת נגישות — בנייה עצמאית במקום widget, ושלושה פערי רמה A שסריקה אוטומטית לא יכולה לתפוס (Phase 6.A)
**תאריך**: 2026-09-05

**רקע**: issue #40 נוסח כ"בר/תפריט נגישות (חובה חוקית)". תקנה 35 לתקנות שוויון זכויות לאנשים עם מוגבלות (התאמות נגישות לשירות), תשע"ג-2013 מחייבת נותן שירות באינטרנט להנגיש את האתר לפי **ת"י 5568 ברמה AA**, ולפרסם **הצהרת נגישות**. היא **אינה** מחייבת בר צף — הבר הוא קונבנציה שוקית. ההבחנה הזו קובעת את סדר העבודה: קודם התאימות עצמה, אחר כך ההצהרה, ורק אז הבר.

**החלטה 1: לבנות את הבר בבית, לא להתקין widget צד-שלישי.** ה-UI כולו מבוסס Radix primitives, שהסמנטיקה שלהם נכונה מלכתחילה. overlay של צד-שלישי עובד ע"י הזרקת ARIA מעל DOM קיים — כלומר על DOM תקין הוא יכול רק להזיק, והוא גם מוסיף script של צד-שלישי לכל עמוד. כל מה שהבר עושה מתבטא כאטריביוט על `<html>` וכללי CSS ב-`globals.css` (`data-a11y-contrast`, `data-a11y-links`, `data-a11y-focus`, `data-a11y-motion`, ו-`--a11y-font-scale`) — אפס נגיעה באלמנטים בודדים.

**החלטה 2: ניגודיות גבוהה היא שכבה מעל `ThemeProvider`, לא מנגנון theme מתחרה.** `[data-a11y-contrast="high"]` דורס את סט הטוקנים ומוגדר **אחרי** `:root` ו-`.dark` ב-`globals.css`: הוא בעל אותה ספציפיות כמו `.dark` (0,1,0), אז סדר המקור הוא מה שמכריע, ו-`.dark[data-a11y-contrast="high"]` (0,2,0) גובר על שניהם. זו הסיבה שהרודמאפ מיקם את 6.A **אחרי** 6.4 — בלי ה-`ThemeProvider` היה צריך כאן מנגנון מקביל.

**החלטה 3: פקדי טופס נטיביים בלבד בתוך הבר.** רדיו לגודל טקסט ו-checkbox לכל מתג, בתוך `<fieldset>` עם `<legend>` — בלי מימוש custom של radiogroup או switch. כך הסמנטיקה, ניווט המקלדת וההכרזה לקורא מסך מגיעים מהדפדפן ולא יכולים להישבר בעדינות. **הרכיב שמיועד למשתמשים עם מוגבלות הוא הרכיב היחיד שאין לו תירוץ ל-widget custom.**

**החלטה 4: סקריפט inline חוסם ל-preferences**, באותה רוח כמו next-themes ומסיבה חזקה יותר: משתמש שקבע טקסט 150% כי אינו קורא ב-100% לא אמור לראות פריים אחד ב-100%. הוא עטוף ב-try/catch לאורך כל הדרך (localStorage זורק ב-Safari פרטי) ולעולם לא יכול למנוע רינדור.

**שלושת הפערים שהסריקה של Phase 6.4 עברה בהצלחה אף שהם כשלים אמיתיים ברמה A** — זה הלקח המרכזי של השלב:
1. **לא היה `<main>` בשום עמוד מוגן.** `(protected)/layout.tsx` עטף את התוכן ב-`div` רגיל. כלל `bypass` של axe מסתפק בקיום כותרות, וכלל `region` מתויג best-practice ולכן הוחרג בכוונה.
2. **לא היה קישור דילוג.** אותה סיבה בדיוק.
3. **לכל עמוד באפליקציה היה אותו `<title>` — "שוברים"** (WCAG 2.4.2, רמה A). כלל `document-title` של axe בודק רק שה-title אינו ריק. התיקון הוא `title.template` בשורש + `metadata` לכל מסלול; לשישה עמודים שהם Client Components נוספו `layout.tsx` מינימליים, כי Client Component לא יכול לייצא `metadata`.

**ממצא ניגודיות אמיתי שנחשף מהרחבת הסריקה מ-2 עמודים ל-8**: `--destructive` היה `oklch(0.577 0.245 27.325)`, וה-variant של shadcn מרנדר `text-destructive` על `bg-destructive/10` — צירוף שנמדד **3.99:1**, כשל WCAG 1.4.3 AA על כל כפתור מחיקה/חסימה באפליקציה (14 שימושים). הוא לא נתפס קודם כי על רקע לבן נקי הוא 4.76:1 ועובר, ולשני העמודים שנסרקו עד כה לא היה פקד הרסני. הוערך מחדש ל-`oklch(0.52 0.22 27.325)` — 5.01:1 על הגוון ו-6.02:1 על לבן. הפלטה הכהה נמדדה 5.30:1 ולא שונתה.

**ממצא שנדחה במכוון, ומתועד כדי שלא ייבדק מחדש**: הסריקה של `/settings` דיווחה 3.69:1 על כפתור הרענון ב-`ChannelLinksSection`. הצבע `#858585` הוא בדיוק `--foreground` (`#0a0a0a`) תחת `disabled:opacity-50` — כלומר הכפתור היה **מושבת** בזמן הסריקה, כי `disabled={loading}` וטעינת הערוצים עדיין רצה. WCAG 1.4.3 פוטר במפורש רכיבי ממשק לא-פעילים, אז זה ארטיפקט של סריקת מצב חולף ולא פגם בפלטה. התיקון הוא בטסט (המתנה לסיום הטעינה לפני הסריקה), לא בצבע. **הכשל היה נראה כמו flake ואובחן נכון רק אחרי ש-`tests/e2e/helpers/a11y.ts` שודרג לדווח את `failureSummary` של axe** — הסלקטור לבדו לא היה בר-פעולה.

**החלטה 5: הצהרת נגישות שמצהירה מה שלא נבדק.** `/accessibility` מונה במפורש שבדיקת NVDA ידנית טרם הושלמה, שתמונות שמשתמשים מעלים מקבלות תיאור כללי, ושתוכן שמפיק מודל שפה ב-`/chat` אינו ניתן לבדיקה מראש. הצהרה שמתיימרת לתאימות מלאה שלא הודגמה גרועה מהיעדר הצהרה — היא המסמך שמולו תימדד תלונה.

**מה עדיין פתוח**: מעבר NVDA ידני ו-Lighthouse מול ה-URL החי; `/admin` אינו מכוסה בסריקה האוטומטית כי הוא דורש `adminRoles` שאין ל-uid שנוצר בטסטים; והחלטה אם נדרש מינוי רכז נגישות תלויה בסיווג העסקי ואינה שאלה הנדסית.

## 58. כותרת דביקה בכל האתר, תפריט בעמודי המידע, ובר הנגישות במרכז האנכי (Phase 6.B)
**תאריך**: 2026-09-05

**רקע**: בדיקה ידנית של תוצרי Phase 6.A (ADR #57) העלתה שני ליקויים. הראשון קוסמטי: הבר ישב בפינה התחתונה. השני מהותי, ונוצר דווקא ע"י התיקון של 6.A — ה-`SiteFooter` החדש הפך את `/accessibility`, `/privacy` ו-`/terms` לנגישים מכל עמוד (כנדרש בתקנה 35), אבל שלושתם יושבים תחת `(public)/` שאין לו layout, בעוד `<Header />` מורכב רק ב-`(protected)/layout.tsx`. כלומר המשתמש הגיע לעמוד ההצהרה **ללא שום דרך חזרה** מלבד כפתור ה-back. הנגשת עמוד שאי אפשר לצאת ממנו היא תיקון חלקי.

**החלטה 1: קומפוננטת `Header` אחת עם prop של הקשר, לא שתי קומפוננטות.** `context="app"` מרנדר בדיוק את המרקאפ הקודם, בלי הסתעפות auth — האזור המחובר כבר עבר redirect ב-layout, אז תמיד יש משתמש ואפס שינוי התנהגות. `context="public"` מסתעף לפי `useAuth()`. שכפול קומפוננטה היה מפצל את קישורי הניווט ואת תפריט האווטאר לשני מקומות שהיו נפרדים עם הזמן.

**החלטה 2: הענף הציבורי נפתר בלקוח, לא ב-`getSessionUid()`.** `getSessionUid` קורא ל-`cookies()`, מה שהיה הופך את שלושת עמודי המידע מ-static ל-dynamic ומוסיף `adminAuth.verifySessionCookie` (קריאת רשת, `checkRevoked: true`) לכל בקשה — על backend של `cpu: 1, memoryMiB: 512, maxInstances: 2`, ודווקא עבור העמודים שסורקים חיצוניים מושכים. מה שהענף מחליט הוא **קוסמטי בלבד**; השער האמיתי הוא `src/proxy.ts` ו-`firestore.rules`. אומת ב-`next build`: שלושתם נשארו `○ (Static)`. המחיר הוא חלון קצר שבו ה-auth עדיין נטען, ולכן הענף מרנדר placeholder בגובה קבוע ולא "התחברות" — הצגת "התחברות" למשתמש מחובר הייתה הבהוב של מצב שגוי, וה-placeholder גם מונע קפיצת גובה.

**החלטה 3: `sticky` מותנה בגובה viewport.** הכותרת היא `static` כברירת מחדל והופכת ל-`sticky top-0` רק מ-`min-height: 480px`. כותרת דביקה גוזלת גובה קבוע, וב-zoom 400% או ב-`--a11y-font-scale: 1.5` היא בולעת נתח משמעותי מהמסך — זה בדיוק פריט ה-reflow ש-`docs/ACCESSIBILITY.md` עוקב אחריו. `bg-background` נוסף כי הכותרת הייתה שקופה ותוכן היה נקרא דרכה בגלילה; הוא טוקן ולא צבע קשיח, כדי שמצב הניגודיות הגבוהה יחול גם עליה. `z-40` — מתחת לבר הצף ול-`PopoverContent` (`z-50`) ומתחת ל-`SkipLink` (`z-[100]`).

בנוסף: `#main-content { scroll-margin-top: 4rem }` ב-`globals.css`. קישור הדילוג קופץ לעוגן, וכותרת דביקה הייתה מכסה את היעד. כלל אחד ולא class לכל `<main>` (יש אחד בכל עמוד ציבורי, ב-`(protected)/layout.tsx` וב-`not-found.tsx`), ב-`rem` כדי שיגדל יחד עם סקאלת הפונט.

**החלטה 4: route group `(legal)` ולא layout על כל `(public)`.** `/` ו-`/invite/[code]` נשארים בלי כותרת בכוונה — דף הנחיתה כבר מציג "שוברים" כ-`<h1>` עם כפתורי ההתחברות בגוף העמוד, ועמוד ההזמנה הוא מסלול המרה ממוקד. route group אינו משנה URL, אז אף מסלול, קישור או טסט לא זז.

**החלטה 5: פאנל הבר נפתח על הציר האופקי.** הבר עבר ל-`fixed end-4 top-1/2 -translate-y-1/2`. עם מפעיל ממורכז אנכית, `side="bottom"` (ברירת המחדל) משאיר לפאנל בן ~340px רק חצי מסך — התנגשות ודאית בכל טלפון. `side="right"` מנצל את מלוא הגובה. **`side` ב-Radix פיזי ולא לוגי**: `@radix-ui/react-popper` 1.3.7 בונה `desiredPlacement = side + align` בלי שום טיפול ב-`dir`, ואינו מכיר `inline-start`/`inline-end`. `dir="rtl"` קבוע ב-root layout, ולכן `end-4` הוא הקצה הפיזי השמאלי ו-`"right"` הוא כיוון מרכז העמוד. יש הערה בקוד, כי זה נראה כמו טעות למי שקורא אותו בלי ההקשר.

**נבדק ולא היווה בעיה**: ה-base של `Button` נושא `active:not-aria-[haspopup]:translate-y-px`, אבל `PopoverTrigger` קובע `aria-haspopup="dialog"` — הווריאנט מוחרג ולא דורס את ה-`-translate-y-1/2`. וכן: אין `overflow-hidden`/`overflow-clip` על אף אב, כך ש-`position: sticky` לא נכשל בשקט.

**כיסוי**: `tests/e2e/accessibility.spec.ts` — נוכחות הכותרת בשלושת עמודי המידע ודביקות אחרי גלילה, הענף הלא-מחובר (קישור התחברות, בלי ניווט האפליקציה), הענף המחובר (ניווט מ-`/privacy` ל-`/cards` — הרגרסיה שבגללה השלב קיים), מרכוז אנכי של הבר, ופתיחת הפאנל בתוך גבולות ה-viewport. סריקת ה-axe הורחבה מ-`/accessibility` בלבד לשלושת עמודי המידע, כי כולם קיבלו landmark `banner` וניווט שני.

## 59. למה אין באנר עוגיות — פטור ההכרחיות, ושני נמעני צד-שלישי שהיו חיים ולא מוצהרים (Phase 6.C)

**תאריך**: 2026-09-05

**רקע**: עלתה השאלה אם החוק שמחייב הצהרה על עוגיות ואישור/דחייה שלהן רלוונטי לפרויקט. הבדיקה שנעשתה כדי לענות עליה מצאה משהו אחר, וחמור יותר, מזה שנשאל עליו.

**מה החוק דורש**: ePrivacy Directive 2002/58/EC סעיף 5(3) מחייב הסכמה לפני אחסון או קריאה של מידע במכשיר, **למעט** מה שהכרחי לחלוטין לשירות שהמשתמש ביקש במפורש; ה-GDPR רק קובע מה נחשב הסכמה תקפה. בישראל אין סעיף ייעודי לעוגיות — החובה נגזרת מסעיף 11 לחוק הגנת הפרטיות (חובת **יידוע**) ומתיקון 13 שנכנס לתוקף ב-14.8.2025. כלומר החוק דורש הסכמה עבור **מעקב, אנליטיקס ופרסום**, לא עבור עוגיות ככלל.

**מה נמצא בפועל** (אומת בקוד, לא הונח): עוגייה אחת, `__session` (`src/actions/auth.ts`), HttpOnly, נוצרת רק בהתחברות; IndexedDB של Firebase Auth; `localStorage` להעדפות סרגל הנגישות שהמשתמש עצמו בחר; `sessionStorage` לשרידות ה-redirect של OAuth; ועוגיית `_GRECAPTCHA` של Google מ-App Check (`src/lib/firebase/appCheck.ts`). אפס אנליטיקס, אפס פרסום, אפס פיקסלים. Heebo מגיע דרך `next/font/google`, שמוריד ב-build ומגיש מהדומיין שלנו — אין קריאת runtime ל-Google. `curl -D -` מול הפרודקשן על `/` ועל `/privacy` מחזיר **אפס `Set-Cookie`**.

**החלטה 1: אין באנר עוגיות, וזו החלטה ולא השמטה.** כל פריט ברשימה למעלה הוא הכרחי או יזום ע"י המשתמש, כלומר בתוך הפטור. באנר מעל אחסון הכרחי אינו רק מיותר — הוא מזיק, כי הוא מאמן משתמשים ללחוץ "אשר" בלי לקרוא, ובקשת הסכמה לדבר שאינו דורש אותה מבוקרת במפורש ע"י גופי הפיקוח. `_GRECAPTCHA` הוא הפריט היחיד שראוי לדיון (הוא צד שלישי, ונטען גם למבקר אנונימי) — הוא נשאר בפטור בהיותו מנגנון מניעת הונאה בלבד, ומוצהר במפורש במדיניות.

**החלטה 2: השער להסכמה אמיתית הוא GA4, לא רגע אחר.** שכבה 3 של Phase 9.6 ברודמאפ היא הנקודה שבה מנגנון הסכמה הופך לחובה, והוא חייב להיות opt-in — בלי ירי אירוע אחד לפני קליק. תועד גם ב-`docs/ROADMAP.md` בגוף אותה שכבה, כי שם מסתכלים לפני שמממשים.

**החלטה 3: הפער האמיתי היה גילוי, ולא הסכמה — ובשני מקומות שאינם עוגיות.** `grep` על `privacy/page.tsx` החזיר אפס התאמות ל"עוגי", ל-`WhatsApp`, ל-`Meta` ול-`Anthropic`. כלומר המדיניות לא הזכירה **שתוכן השיחות עובר ל-Anthropic** — ומ-ADR #36 הוא עשוי לכלול `cvv`/`barcodeOrCode` בטקסט גלוי — ולא **שהוא עובר דרך השרתים של Meta** כשמקושר ערוץ WhatsApp. `docs/PRIVACY.md` כבר סימן את השני כחוב פעיל בפרודקשן שלא הוסדר; שלב זה סוגר אותו. שלושת הנמענים מוצהרים עכשיו בשמם, וה-WhatsApp מנוסח כמה שהוא — opt-in שהמשתמש יוזם ויכול לנתק.

**החלטה 4: העלאת `PRIVACY_POLICY_VERSION` ל-`2026-09-05`, כלומר re-consent לכל משתמש קיים.** תוספת נמען צד-שלישי היא שינוי מהותי, ו-`docs/PRIVACY.md` דורש במפורש העלאת גרסה במקרה כזה. נבדק שזה בטוח לטסטים: `tests/e2e/helpers/auth.ts` מזהה את הדיאלוג דינמית ולא נשען על מחרוזת גרסה; `tests/rules/firestore.test.ts` מחזיק אותה כ-fixture בלבד (ה-rules לא בודקים את השדה) ועודכן בכל זאת כדי שלא יסתור את הקוד.

**החלטה 5: הנמענים נכתבים גם ב-`ConsentBanner` עצמו, לא רק מאחורי הקישור.** הבאנר הוא רגע ההסכמה בפועל, וקישור למדיניות לבדו אינו "מיודע" כשהשינוי המהותי הוא מי עוד רואה את המידע. שני משפטים, במכוון — קיר טקסט בדיאלוג חוסם נלחץ בלי שנקרא, וזה מבטל את התכלית.

**כיסוי**: `tests/e2e/public.spec.ts` — מבקר שלא התחבר מקבל אפס עוגיות ואפס בקשות לדומיינים של מעקב, ו-`/privacy` מכיל את שתי הכותרות החדשות ואת שלושת הנמענים. הטסט הראשון הוא העיקר: הוא הופך את ההחלטה הזו מנייר לאכיפה, וייכשל ביום שבו מישהו יוסיף אנליטיקס בלי לעבור כאן.

---

## 60. כפתור "לאזור האישי" אחרי פעולה בוואטסאפ — זיהוי בקוד ולא דרך המודל (issue #62)

**תאריך**: 2026-09-05

**רקע**: issue #62 ביקש שהודעת סיכום של הבוט (אחרי הוספה/מחיקה/עריכה/הרשמה) תצורף בלינק לאזור האישי, עם אילוץ מפורש: **בלי להשפיע על ה-context של ה-LLM ובלי להעלות את צריכת הקרדיטים**. התשתית עצמה כבר נבנתה ב-issue #66 — `ChannelReply` עם שדה `cta`, ו-`sendWhatsAppCtaUrl` ששולחת הודעת `interactive`/`cta_url` (כפתור אמיתי, לא URL בתוך הטקסט). השלב הזה רק מרחיב את השימוש בה.

**אין כאן חסם template של Meta.** הבוט **עונה** להודעה נכנסת, כלומר בתוך חלון 24 השעות שבו טקסט חופשי והודעות `interactive` מותרים ללא template מאושר. השגיאה `131047` שאומתה בפרודקשן בשלב 5.5.c חלה על **יזום** הודעה בלבד (וזה מה שיחסום את Phase 7), ולא רלוונטית כאן.

**החלטה 1: הזיהוי דטרמיניסטי, ב-`onToolCall`, ולא דרך ה-system prompt.** `channelChat.ts` אוסף את שמות הכלים שהסבב הריץ דרך callback שכבר היה קיים ב-`agentLoop.ts` ופשוט לא היה בשימוש שם, ומחליט לבד. שלוש תוצאות שהן בדיוק האילוץ מה-issue: אין נגיעה ב-`systemPrompt.ts` ואין tool חדש → אפס טוקני input נוספים; אין קריאת API נוספת; וה-`cta` הוא **שדה נפרד** ב-`ChannelReply` שלעולם לא נכנס ל-`reply.text` ולכן לא נשמר ב-`chatSessions` ולא חוזר כטוקנים בסבב הבא. אילו הפיצ'ר היה מיושם כהוראה למודל ("צרף לינק כשביצעת פעולה") הוא היה גם עולה טוקנים בכל סבב וגם היה נתון להזיה.

**החלטה 2: allowlist של כלי כתיבה (`MUTATING_TOOL_NAMES` ב-`src/lib/mcp/toolEffects.ts`), לא "כל מה שאינו קריאה".** שבעה כלים: `createCard`, `updateCard`, `deleteCard`, `logUsage`, `deleteUsageEntry`, `updateBalance`, `createList`. שלושת הנותרים (`listCards`, `getCard`, `listCardLists`) הם רוב התעבורה בוואטסאפ ("מה היתרה שלי?"), וכפתור על כל אחת מהתשובות האלה הופך רעש במקום קיצור דרך. **מחיר ההחלטה, ונאמר במפורש**: כלי כתיבה חדש שלא יתווסף לרשימה פשוט לא יקבל כפתור, בשקט — אין כשל רועש. המיטיגציה היא הערה בגוף `createMcpServer` ליד בלוק ה-`registerTool`, שורה ב-`docs/CHATBOT.md`, ובדיקה ב-`tests/unit/toolEffects.test.ts` שמקבעת את עשרת השמות הקיימים כך ששינוי שם ישבור טסט.

**החלטה 3: היעד הוא `/dashboard`.** האזור האישי עצמו — מספר הכרטיסים הפעילים והיתרה הכוללת — ולא `/cards` (רשימות בלי הסיכום) ולא `/settings` (ניהול הקישור, לא "בוא תראה מה השתנה"). `buildDashboardUrl()` נוסף ל-`src/lib/appUrl.ts` לצד `buildListInviteUrl` הקיים; אין קונפיג חדש, `NEXT_PUBLIC_APP_URL` כבר מוגדר בפרודקשן מ-issue #58. אם ה-session פג, `src/proxy.ts` מפנה להתחברות עם `?next=` וחוזר לשם.

**החלטה 4: `REPLY_LINKED` מקבל את הכפתור, `REPLY_NOT_LINKED` נשאר על `HOME_CTA`.** קישור מספר מוצלח הוא ה"הרשמה" שה-issue מונה, והרגע הטוב ביותר להוביל לחשבון. לעומת זאת שולח שאינו מקושר צריך את שורש האתר כדי להתחבר ולהגיע להגדרות — dashboard היה מפנה אותו למקום שאין לו גישה אליו.

**החלטה 5: תשובה ארוכה מ-1024 תווים נשלחת כטקסט רגיל, בלי כפתור.** `sendWhatsAppCtaUrl` חותכת את הגוף ל-`MAX_INTERACTIVE_BODY_LENGTH` (1024) מול 4096 ב-`sendWhatsAppText`. עד היום זה לא הזיק כי `cta` ישב רק על מחרוזת קבועה קצרה; מרגע שהוא מוצמד ל**תשובת מודל**, הפיצ'ר הזה היה אוכל את סוף התשובה בשקט. ה-route בודק את האורך לפני שהוא בוחר שולח — **לאבד את הכפתור עדיף מלאבד את התוכן.**

**כיסוי ומגבלתו**: `tests/unit/toolEffects.test.ts` מכסה את ההחלטה. את הכפתור עצמו **אי אפשר** לאמת ב-E2E: הטסטים רצים בלי credentials יוצאים, `sendWhatsAppCtaUrl` מחזירה `false` עם warning ולא משאירה עקבה נצפית. האימות בפועל הוא `npm run whatsapp:sim`, שמריץ את `handleInboundChannelMessage` האמיתי ומדפיס את ה-`cta` שחזר.

## 61. מועדוני חברות: היררכיה דו-שכבתית, קטלוג סגור, וחברות שמצביעה על כרטיס
**תאריך**: 2026-09-06

**רקע**: המשתמש רוצה שהאפליקציה תדע אילו כרטיסי מועדון צרכנות יש ברשותו (מפעל הפיס, בהצדעה), כדי שבשלב הבא אפשר יהיה לשלוף מהרשת את ההטבות של אותם מועדונים, להציג לכל משתמש רק את הרלוונטיות לו, ולתת לבוט לדבר עליהן. שלב 1 — מבנה הנתונים וה-UI — הוא מה ש-ADR הזה מתעד. הפיצ'ר היה greenfield לחלוטין: אפס אזכורים של `מועדון`/`club`/`הטבה` בקוד או בתיעוד.

**החלטה 1: שתי רמות — מועדון וכרטיס — והחברות מצביעה על הכרטיס.** תחת מועדון אחד יכולים לחיות כמה דרגי כרטיס ("מפעל הפיס" מול "מפעל הפיס VIP"), וההטבות נבדלות ביניהם. מודל חד-שכבתי, שבו המשתמש מסמן "מועדון", היה מאבד בדיוק את ההבחנה שבגללה הפיצ'ר קיים: הוא היה מציג ל-VIP את הטבות הרגיל ולהפך. לכן `clubs` ↔ `clubCards` ↔ `clubMemberships`, כשהחברות מצביעה תמיד על `clubCardId`.

**החלטה 2: `clubCards` הוא collection שטוח עם `clubId`, לא subcollection של `clubs`.** ה-UI טוען את כל הקטלוג במכה אחת ומקבץ בזיכרון. subcollection היה מחייב `collectionGroup()`, ואיתו רשומת `fieldOverrides` ב-`firestore.indexes.json` — אינדקס שהאמולטור **לא** תופס כשהוא חסר, כי הוא ממציא אינדקס לכל שאילתה שמבקשים ממנו (ADR #33, שנולד מ-500 בפרודקשן בדיוק מהסיבה הזו) — ורול `{path=**}` נפרד ב-Rules. בקטלוג בגודל עשרות מסמכים אין שום רווח שמצדיק את שני החובות האלה.

**החלטה 3: המועדון נגזר מהקטלוג ולא נשמר על מסמך החברות.** דנרמליזציה של `clubId` לתוך `clubMemberships` הייתה נוחה לשאילתות עתידיות, אבל היא שדה שהלקוח כותב — ולכן שדה שהלקוח יכול לזייף. ה-Rules לא יכולים לאמת אותו מול `clubCards` בלי `get()` על מסמך אחר. הקטלוג קטן, נקרא ממילא, והוא מקור אמת יחיד שהלקוח לא יכול לכתוב אליו.

**החלטה 4: מזהה דטרמיניסטי `${uid}_${clubCardId}`, נאכף ב-Rules, ובלי `update`.** האכיפה (`membershipId == request.auth.uid + '_' + request.resource.data.clubCardId`) הופכת כפילות לבלתי-ניתנת-לייצוג, ולכן הסימון והביטול הם `setDoc`/`deleteDoc` עיוורים — בלי read-then-write שיחפש שורה קיימת. `update` חסום כי אין במסמך שדה בר-שינוי: שינוי דעה הוא מחיקה, וסימון מחדש הוא create חדש, כך ש-`createdAt` נשאר כנה.

**החלטה 5: קטלוג סגור — אין מועדון או כרטיס שהמשתמש יוצר.** `allow read: if isSignedIn()`, `allow write: if false` על שני ה-collections; הזריעה היא `scripts/seed-clubs.ts` דרך Admin SDK. זה שונה במכוון מ-`categories`, שמערבב ברירות מחדל מערכתיות עם שורות פר-משתמש: קטגוריה מותאמת אישית עובדת מצוין כי המשתמש הוא שנותן לה משמעות, בעוד מועדון שהמערכת לא מכירה לא יוכל להביא הטבות בשכבה הבאה — כלומר היה מייצר תיבת סימון שלא עושה כלום.

**החלטה 6: החברות אינה זכאות.** שום דבר כאן אינו מאומת מול המועדון; זו הצהרה עצמית שכל תפקידה לסנן מה יוצג. אם אי-פעם תידרש חברות מאומתת, היא לא תוכל לחיות במסמך שהמשתמש כותב אליו — אותו נימוק שבגללו `blocked` לא יושב על `users/{uid}` (ראו `docs/DATA_MODEL.md`).

**החלטה 7 (2026-09-06, מילוי הקטלוג): לוגו לכל מועדון — כקובץ מקומי ב-`public/clubs/`, לא hotlink.** שבעת המועדונים נזרעים עם `logoUrl` שמצביע על קובץ שמור ברפו. הפיתוי היה לקשר ישירות ל-CDN של כל מועדון: אפס משקל ברפו, ותמיד מעודכן. נדחה — זה היה הופך את `/clubs` לעמוד ששולח את הדפדפן של כל מבקר לשבעה נמענים חדשים, כלומר גילוי חדש ב-`docs/PRIVACY.md` ו-bump ל-`PRIVACY_POLICY_VERSION` (ADR #59) עבור תמונה דקורטיבית, ובנוסף תלות בזמינות של אתר צד-שלישי לרינדור העמוד שלנו. `website` הפך ל-nullable באותה הזדמנות: לאשמורת אין אתר חי (`itu.org.il` מושהה, `ashmoret.co.il` מפנה לעמוד ריק), וקישור מת גרוע מהיעדר קישור. הלוגו מרונדר על אריח לבן קבוע שאינו עוקב אחרי ה-theme — יש באפליקציה dark mode ומצב ניגודיות גבוהה, ושניהם היו הופכים לוגו על רקע לבן לריבוע חיוור או משנים סימן מסחרי שאינו שלנו לשנות. `logoUrl: null` נופל לאריח-אות בצבע המועדון, כדי שהוספת מועדון לא תהיה תלויה גם במציאת artwork.

**החלטה 8: דרגי מפעל הפיס הם רגיל/VIP, אף שפיס פלוס מפרסם ארבעה.** מודל הזכאויות בפועל הוא כסף / זהב / זהב+ / פלטינום (לפי ותק ומספר כרטיסי מנוי), וההטבות המסומנות "VIP+" שמורות לשניים העליונים. הפישוט נבחר ביודעין ע"י בעל המוצר. **זו החוב הידוע היחיד שנכנס לקטלוג**: ברגע ששכבה 10.2 תתאים הטבה לדרג, הרשימה ב-`scripts/clubs.data.ts` תצטרך לגדול לארבעה, וחברויות קיימות על `mifal-hapais-regular`/`-vip` יצטרכו מיפוי.

**החלטה 9: הקטלוג נשמר ב-`scripts/clubs.data.ts`, בנפרד מהסקריפט שזורע אותו.** `scripts/seed-clubs.ts` מריץ `main()` בזמן import, ולכן ייבוא שלו מטסט היה כותב ל-Firestore. ההפרדה מאפשרת ל-`tests/unit/clubCatalog.test.ts` לאמת את הקטלוג מול המציאות — בעיקר ש**כל `logoUrl` מצביע על קובץ שקיים בפועל תחת `public/`**. זו לא קפדנות: ה-E2E רץ במכוון מול fixture ולא מול הקטלוג האמיתי (החלטה קודמת באותו ADR), ולכן טעות הקלדה בנתיב לוגו הייתה עוברת typecheck, lint, build ו-E2E, ומתגלה רק כתמונה שבורה בפרודקשן.

**החלטה 10 (2026-09-06, Phase 10.1.b): הקטלוג נערך מפאנל הניהול — ו-`allow write: if false` נשאר בתוקף.** החלטה 5 קבעה קטלוג סגור, וההנמקה שלה — "מועדון שהמערכת לא מכירה לא יוכל להביא הטבות" — עדיין עומדת. מה שהשתנה הוא **מי** יכול לפתוח אותו: לא הלקוח, אלא אדמין מאומת בשרת. `/admin/clubs` כותב דרך `src/actions/adminClubs.ts` → `requireAdmin()` → `src/lib/services/adminClubs.ts` → Admin SDK, בדיוק כמו חסימת משתמש (ADR #42/#44). ה-Rules לא נגעו: שום דפדפן, כולל של האדמין, לא כותב ל-`clubs`/`clubCards`. כל שינוי נרשם ב-`adminAuditLog`, שקיבל לשם כך `targetId` — היעד הראשון כאן שאינו אדם.

המניע היה קונקרטי: הזריעה הראשונה בפרודקשן דרשה `scripts/seed-clubs.ts` עם משתני `FIREBASE_ADMIN_*` אמיתיים, כלומר חילוץ מפתח פרטי מ-Secret Manager אל קובץ `.env` על תחנת עבודה — טקס שצריך לחזור על עצמו בכל הוספת מועדון. הכפתור "החלת הקטלוג המובנה" מחליף אותו: `src/lib/services/clubCatalogData.ts` (שעבר מ-`scripts/` ל-`src/` בדיוק בשביל זה) מוחל בלחיצה, ואף אחד לא נוגע במפתח.

**נבדק ונדחה — מחיקה קשה של דרג שיש לו מחזיקים.** `clubMemberships` מצביע על `clubCardId`, ומחיקת הדרג הייתה משאירה את ההחזקה יתומה: היא נשארת ב-Firestore ובייצוא הנתונים (עם שמות `null`, מקרה שהייצוא כבר מטפל בו), אבל מפסיקה בשקט להיות בעלת משמעות ואי אפשר להסביר למשתמש למה. לכן `deleteClubCard`/`deleteClub` מריצים `count()` ומסרבים אם יש ולו מחזיק אחד; הכפתור בפאנל מושבת מראש ומציג את המספר, כדי שהסירוב לא יהיה הפתעה. הדרך להוריד דרג מהאוויר היא `isActive: false`, שמשאירה כל החזקה על כנה.

**החלטה 11: לוגו שמועלה מהפאנל נשמר ב-Firebase Storage תחת `clubLogos/{clubId}`, וההעלאה עוברת ב-Server Action ולא ב-client SDK.** `storage.rules` שומר על `allow write: if false` בנתיב הזה — נכס קטלוג הוא לא הדאטה של אף אחד, ואם דפדפן היה יכול לכתוב לשם, מספיק היה באג הרשאות אחד כדי להחליף לוגו של מועדון לכל המשתמשים. המחיר: הקובץ עובר דרך שרת Next, ולכן נחסם ל-512KB — מתחת למגבלת ה-1MB המובנית של Server Action, כדי שקובץ גדול מדי ייתן הודעה בעברית ולא 413 אטום. ה-URL נבנה עם `firebaseStorageDownloadTokens` ולא עם `makePublic()`: הפיכת אובייקט לציבורי היא כתיבת ACL, שנכשלת על bucket עם uniform bucket-level access, בעוד token עובד בשני המצבים — וטוקן חדש בכל העלאה משמש גם כ-cache-busting, כי נתיב האובייקט קבוע.

**החלטה 12 (2026-09-06): כל פעולה הרסנית בפאנל המועדונים עוברת דיאלוג אישור אחד ומשותף.** בגרסה הראשונה רק מחיקת מועדון ביקשה אישור, בנימוק של פרופורציונליות (ADR #42 החלטה 5: רק הפעולה הכי משמעותית מאשרת). בעל המוצר ביקש להרחיב את זה גם למחיקת דרג ולהסרת לוגו, וזה הנימוק שגובר: הבדיקה של `assertNoMemberships` מגנה על המשתמשים, לא על האדמין — היא מונעת החזקה יתומה, אבל לא מונעת מחיקה בטעות של דרג שאף אחד עוד לא סימן, ולא נוגעת בלוגו בכלל. שלושת המקרים מוזנים לאותו `PendingConfirm` ב-`ClubCatalogManager.tsx` (כותרת, משפט על מה נאבד, ותווית לכפתור) במקום שלושה דיאלוגים כמעט-זהים שהיו נפרדים בניסוח. הסרת לוגו מנוסחת במכוון בלי "אינה ניתנת לביטול" — הקובץ נמחק, אבל התיקון הוא העלאה מחדש, וניסוח חמור מדי היה משטח את ההבדל מול מחיקת מועדון.

**מחיר ההחלטה, ונאמר במפורש**: הקטלוג הוא עכשיו נכס שדורש תחזוקה — מועדון חדש, או דרג חדש תחת מועדון קיים, מחייב או עריכה ב-`/admin/clubs` (מיידית, בלי deploy), או עריכת `src/lib/services/clubCatalogData.ts` והחלה מחדש. זה המחיר של החלטה 5, והוא שולם ביודעין. הזריעה אידמפוטנטית ואינה מוחקת מועדון שהוסר מהרשימה — הדרך להוריד מועדון מהאוויר בלי להשמיד חברויות קיימות היא `isActive: false`.

**נבדק ונדחה**: (א) מערך `clubCardIds: string[]` על `users/{uid}` — `allow update: if isOwner(uid)` שם אינו מגביל שדות, ובעיקר אי אפשר לשאול "מי מחזיק בכרטיס X", שאילתה שתידרש בשכבה 2 כדי לדעת אילו מקורות בכלל צריך לסרוק; (ב) `benefitsSourceUrl` כשדה בקטלוג כבר עכשיו — נדחה כשדה מת עד שתתקבל ההחלטה על מנגנון השליפה, וכשיתווסף מקומו הוא על `clubCards` ולא על `clubs`, כי להטבות של VIP מקור אחר מאלה של הרגיל.

**מה נשאר פתוח, ובמכוון**: מנגנון השליפה מהרשת — allowlist דומיינים מוכר מול fetch כללי מוקשח (חסימת IP פרטי מפני SSRF, timeout, הגבלת גודל תגובה) — הוא ההכרעה הפתוחה של issue #50 ב-`docs/ISSUES_SPRINT.md`, ו**היא תיכתב כ-ADR נפרד לפני שורת קוד אחת של שליפה**. שלב 1 אינו מבצע אף בקשת HTTP יוצאת, ולכן לא נדרש לו.

---

## 62. סקרייפרים להטבות מועדונים: allowlist קשיח, דריסה מלאה עם שסתום, ותקרה שנקבעת בפאנל

**רקע**: שלב 10.1 הביא קטלוג מועדונים וכרטיסים, ואפס הטבות. שלב 10.2 — שליפת ההטבות מהרשת — היה **חסום** על ההכרעה הפתוחה של issue #50: allowlist דומיינים מוכר מול fetch כללי מוקשח. ADR #61 קבע במפורש שההכרעה תיכתב כ-ADR נפרד "לפני שורת קוד אחת של שליפה". זה ה-ADR הזה.

**מחקר המקורות (נבדק אמפירית מול האתרים החיים, לא נאמד):** חמישה משבעת המועדונים רצים על שתי פלטפורמות white-label משותפות, ולכן ארבעת המועדונים שבהישג יד מכוסים בשלושה פרסרים:

| מועדון | מקור | נגישות |
|---|---|---|
| הוט | `api.hot.co.il/api/website/2.0/getAllBenefits/?page=N&size=30` | פתוח לגמרי. 7,101 הטבות, 237 עמודים (`size` נעול על 30 בשרת) |
| טוב+ | `window.__PRELOADED_STATE__` בכל `/category/{id}` | פתוח |
| פיס פלוס | אותה פלטפורמה (Dolce) בדיוק | פתוח — פרסר אחד לשניהם |
| חבר | `hvr.co.il/bs2/datasets/{giftcard,giftcard_branches,teamimcard_branches}.json` | חלקי — ראו החלטה 4 |
| שווה / בהצדעה | `back.shavve.co.il/api`, `back.behatsdaa.org.il/api` | **סגור**. אותה פלטפורמה (DTS) |
| אשמורת | — | אין אתר חי |

**שווה ובהצדעה נבדקו ונשללו סופית, לא בהנחה**: נחלצו כל 64 נתיבי ה-API מתוך ה-bundle הלא-ממוזער של `hatavot.shavve.co.il` (שהוא, אגב, האתר האמיתי — `www.shavve.co.il` הוא דף נחיתה שיווקי), ותשעת הנתיבים שאינם דורשים משתמש נבדקו מול שני ה-hosts. **כולם 401** — middleware אימות גלובלי, לא הרשאה לכל endpoint. ההבחנה חשובה: אילו רק חלק היו 401, היה שם סדק.

**החלטה 1: allowlist קשיח של hosts, ולא fetch כללי מוקשח.** זו ההכרעה של issue #50. הנימוק המכריע אינו "allowlist בטוח יותר" באופן כללי, אלא ש**במקרה הזה אין בכלל URL שמגיע ממשתמש**: כל בקשה נבנית מתוך `ALLOWED_HOSTS` בקוד ועוד מזהה שהגיע מתשובה קודמת של אותו host. אין endpoint מסוג "תביא לי את ה-URL הזה" בשום מקום בפיצ'ר, ולכן צורות ה-SSRF הקלאסיות (169.254.169.254, 127.0.0.1, file://) אינן מוגנות אלא **בלתי-נגישות**. ה-allowlist הוא מה שמשמר את התכונה הזאת כשהקוד יגדל: אדפטר חדש שמכוון ל-host חדש נכשל בקול ב-`functions/src/benefits/http.ts` במקום להפוך בשקט ל-fetch proxy כללי.

ההקשחות שנלוו: `redirect: "manual"` עם בדיקת ה-host מחדש בכל hop (עם `redirect: "follow"` הקפיצה הייתה מתבצעת לפני שהקוד שלנו רואה אותה — כלומר מסירה את הערובה של ה-allowlist לידי מי ששולט במקור), timeout של 10 שניות, תקרת תגובה של 10MB **הנאכפת תוך כדי קריאת ה-stream** (כי `Content-Length` הוא אופציונלי וניתן לזיוף), התאמה מדויקת של host ולא `endsWith` (אחרת `api.hot.co.il.evil.example` עובר), https בלבד, retry רק על 429/5xx, ומרווח קבוע בין בקשות לאותו host. ה-User-Agent הוא `ShovarimBot/1.0 (+<app-url>)` — מזהה ולא מתחזה לדפדפן.

**עמידה ב-robots.txt נבדקה לכל host בנפרד**, וזה לא פורמלי: `www.hot.co.il/robots.txt` אוסר `/api/`, אבל `api.hot.co.il` הוא host אחר עם robots משלו שהוא `Allow: /` בלי Disallow, ו-`Content-Signal: use=reference` — שזה בדיוק השימוש כאן (הפניה לעמוד ההטבה אצל המועדון). טוב+ ופיס פלוס הם `Allow: /` עם `/category/` מותר במפורש.

**החלטה 2: דריסה מלאה דרך run-stamp, עם שסתום ביטחון שהוא חלק מההחלטה ולא תוספת.** הדרישה היא תמונת מצב בלי שאריות: כל מסמך נכתב עם `scrapeRunId` של הריצה, ובסוף נמחק כל מה ששייך למועדון ונושא stamp אחר. השאילתה (`clubId ==` ועוד `scrapeRunId !=`) קוראת רק את השאריות, לא את כל אוסף המועדון.

הצד השני של המטבע הוא שמקור שיש לו בוקר גרוע היה מוחק קטלוג שלם, בשקט, ב-04:00. לכן ריצה **נעצרת לפני שהיא כותבת או מוחקת** אם לא חזרה אף הטבה, או אם המקור הצטמק מתחת ל-50% מהריצה המוצלחת האחרונה; הסטטוס הוא `aborted` והנתונים הקיימים נשארים על כנם. עלות הסירוב היא יום של נתונים ישנים; עלות הכתיבה היא קטלוג שלם בלי undo.

**ההשוואה היא על `sourceTotal` — כמה הטבות יש במקור — ולעולם לא על כמה נשמרו.** זו לא קפדנות: `written` חסום בתקרה שהאדמין קובע, ולכן הורדת תקרה מ-50 ל-10 הייתה נקראת כקריסה של 80% ומפילה כל ריצה אחרי כל שינוי תקרה. את `sourceTotal` מקבלים מהמטא-דאטה של המקור בלי למשוך הכל (`metaData.totalResults` בהוט, סכום `product_count` ב-Dolce), ולכן הוא נשאר נכון גם כשהתקרה עצרה את המשיכה אחרי שני עמודים. אומת חי: 50 ← תקרה 10 ← 10 נשמרו, 40 נמחקו, `success`.

**החלטה 3: תקרת חילוץ נקבעת מהפאנל, ברמת מועדון וברמת כרטיס, ברירת מחדל 50.** בעל המוצר ביקש את שתי הרמות. הסמנטיקה: תקרת הכרטיס נאכפת ראשונה (כל כרטיס מתמלא עד שלו), ותקרת המועדון חוסמת את האיחוד; למועדון חד-דרגי השתיים מתלכדות והנמוכה גוברת. `0` = ללא הגבלה — ולכן כל קריאה של השדה חייבת להיות `?? default` ולא `|| default`, אחרת מועדון ללא הגבלה נצמד בשקט חזרה ל-50.

התקרה אינה רק בקרת אחסון אלא **בקרת משיכה**: האדפטרים מקבלים `shouldStop()` ועוצרים למשוך ברגע שהתקרות מלאות, ולכן הוט מושכת 2 עמודים בברירת המחדל במקום 237. האדפטר עצמו לא מכיר את מודל התקרות בכלל — הוא מקבל `offer`/`shouldStop` כ-callbacks — כך שאדפטר עתידי למועדון סגור יקבל את האכיפה בחינם.

50 ולא "הכל" כברירת מחדל, כי המקורות גדולים בהרבה ממה שהם נראים (הוט לבדה 7,101), וריצה ראשונה ללא תקרה הייתה מכניסה עשרות אלפי מסמכים ל-Firestore לפני שמישהו ראה אם הפרסור בכלל נכון.

**החלטה 4: `benefitKind: "offer" | "acceptance"` — כי הנתונים של חבר הם לא מה שהם נראים.** קבצי ה-JSON הפתוחים של חבר הם **מדריכי בתי עסק, לא הטבות מתומחרות**: 345 עסקים שמכבדים את הגיפט-קארד ו-1,596 סניפי חבר טעמים, עם קטגוריה, אתר ומגבלות — ובלי מחיר, כי ההטבות המתומחרות של חבר יושבות מאחורי ההתחברות. `internal_link` מפנה ל-`mcc.co.il/mcc_item_new.aspx?id=`, שנבדק ומחזיר 200 עם 6 בתים — stub חסום-אימות, כלומר אין דרך להשלים משם מחיר.

בלי ההבחנה, כ-1,900 שורות בלי מחיר היו יושבות באותו אוסף עם הטבות אמיתיות ונקראות כ"הטבות שהפרסור של המחיר שלהן נכשל" — דבר אחר לגמרי, ומדאיג הרבה יותר. שני הסוגים חיים באותו collection ותחת אותו מנגנון דריסה; שלב 10.3 יציג אותם שונה.

התגלית הזאת גם שינתה את הקטלוג: לחבר יש **שני מוצרי כרטיס** נפרדים עם שתי רשימות בתי עסק שונות, ולכן נוסף `hever-teamim` לצד הכרטיס הקיים. הכרטיס הקיים שמר על המזהה `hever-regular` ורק שמו התעדכן — שינוי המזהה ל-`hever-giftcard` היה מייתם כל `clubMembership` שמצביע עליו, בדיוק הכשל ש-`assertNoMemberships` נועד למנוע.

**החלטה 5: התמונות נשארות ב-CDN של המועדון ולא מועתקות ל-Storage.** זו **חריגה מודעת מהחלטה 7 של ADR #61**, שקבעה לוגו מקומי ולא hotlink. הנימוק שם — לא לשלוח את הדפדפן של כל מבקר לנמענים חדשים בשביל תמונה דקורטיבית — לא מתהפך, אבל הוא נכתב על שבעה לוגואים; כאן מדובר בעשרות אלפי תמונות מוצר שמתחלפות מדי יום, והעתקתן הייתה הופכת את Storage למראה של קטלוג צד-שלישי. `media.dolcemaster.co.il` ו-`cdn.hot.co.il` נבדקו כניתנים ל-hotlink ללא Referer. **זו חשיפה חדשה שצריכה להירשם ב-`docs/PRIVACY.md` כששלב 10.3 יציג את ההטבות למשתמש** — לא בשלב הזה, שבו איש אינו רואה אותן מחוץ לפאנל.

לוגו של חבר, לעומת זאת, נשאר `null`: השדה מכיל שם קובץ בלבד (`logo_shasha.jpg`) ונתיב הבסיס אינו נגיש ציבורית — כל תחילית סבירה תחת `hvr.co.il` ו-`mcc.co.il` מחזירה 404. ניחוש היה שם תמונה שבורה על כל שורה של חבר, ולכן חל אותו כלל שכבר תועד ב-`Club.logoUrl`: אריח-אות עדיף על אייקון שבור.

**החלטה 6: הפאנל קורא בלבד — למעט התקרות.** אין "ערוך הטבה", כי הריצה הבאה תדרוס כל עריכה תוך יום, וכפתור כזה מבטיח שינוי שנעלם בשקט. מה שכן ניתן לעריכה הוא מה ששייך לנו באמת: התקרות (בטפסים הקיימים של המועדון והכרטיס, לא במסך נפרד — התקרה היא תכונה של המועדון) והרצה ידנית.

**החלטה 7: הקוד יושב ב-`functions/`, וההרצה הידנית היא callable.** הסקרייפר חייב לרוץ מהמתזמן, ו-`functions/tsconfig.json` מקבע `rootDir: "src"` (ADR #24), ולכן הוא לא יכול לחיות ב-`src/`. הרצה מתוך אפליקציית Next נשקלה ונדחתה: `apphosting.yaml` מגדיר `maxInstances: 2`, וריצה של דקות על אותו Cloud Run שמשרת משתמשים הייתה חורגת מ-timeout הבקשה ומתחרה על מופעים. הכפתור בפאנל קורא ל-callable `adminScrapeBenefitsNow` — בדיוק הדפוס של `adminDeleteUserNow` (ADR #45), כולל אימות ההרשאה בתוך הפונקציה עצמה, כי callable הוא endpoint ציבורי.

המחיר: הטיפוסים וקבוע ברירת המחדל של התקרה מוכפלים בין `functions/src/benefits/` ל-`src/`. זו אותה כפילות ש-ADR #24 כבר כופה על `GRACE_PERIOD_DAYS`. הכפילות המסוכנת היחידה — רשימת המועדונים הנתמכים — מוגנת בטסט שנכשל אם שני העותקים מתפצלים.

**נבדק ונדחה — התחברות למועדונים הסגורים.** שלוש דרכים נשקלו: חשבון שירות אחד למועדון (מפר כמעט בוודאות את תנאי השימוש, והסודות הם PII אמיתי של אדם), אישורים per-user מוצפנים (שטח תקיפה עצום — אחסון סיסמאות של צד ג' — ובלי מסלול מחיקה נקי), ודפדפן אמיתי שעוקף את Imperva (שביר, יקר, ועדיין דורש אחד משני הראשונים). בעל המוצר בחר במפורש: **שלב זה מכסה את המקורות הפתוחים בלבד.** שווה, בהצדעה ואשמורת רשומים ב-`registry.ts` עם `adapter: null` וסיבה בעברית שהפאנל מציג — במכוון, כי מועדון שנעדר מהרשימה היה נראה בדיוק כמו מועדון שהסקרייפר שלו נשבר.

**נבדק ונדחה — סריקה לעומק בפלטפורמת Dolce.** הגרסה הראשונה עשתה BFS על הקטגוריות והציעה את כל מוצרי הקטגוריה לפני שעברה לבאה. זה נראה נכון אבל היה חסר משמעות: עמוד קטגוריה מחזיר את מוצריו עד `show:1000`, ולכן הקטגוריה הראשונה מילאה תקרה של 50 לבדה, וכל ההטבות שנשמרו נשאו את אותו שם קטגוריה — 50 מתוך 50 בטוב+ תחת "חגי תשרי". התיקון הוא מכסה של 8 מוצרים לקטגוריה במעבר הראשון ואז מעבר שני round-robin על מה שנותר (בלי בקשות רשת נוספות), ובנוסף דחיית קטגוריית דף-הבית לסוף לפי הדגל `is_homepage` של הפלטפורמה עצמה — היא מציגה הכל ולכן לא אומרת דבר על הנושא. אחרי השינוי ההטבות מתפרסות על שבע קטגוריות לכל מועדון.

**מה שנשאר פתוח ובמכוון**: (א) פיצול ארבע רמות הזכאות של פיס פלוס בין רגיל/VIP — החוב הידוע מ-ADR #61 החלטה 8. הקודים הגולמיים נשמרים ב-`sourceMemberTypes`, כך שהפתרון יהיה מיגרציית נתונים ולא scrape חוזר; בינתיים כל הטבה של פיס פלוס משויכת לשני הדרגים, שזו הכלה-יתר ולא הסתרה ממי שזכאי. (ב) חלק מהקטגוריות של Dolce הן תוויות שיווק ולא נושאים ("מומלצים", "לגימלאי", "חגי תשרי") ולכן נופלות ל-`system-other`; `sourceCategory` נשמר גולמי כדי שזה יישאר גלוי ולא ייעלם. (ג) ה-tool ל-MCP מ-issue #50 עצמו טרם מומש — אבל מדיניות ה-fetch שהוא חיכה לה מוכרעת כאן, וכשייכתב הוא ילך לפי `functions/src/benefits/http.ts`.
