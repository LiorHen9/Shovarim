...
| בוט WhatsApp (שיחה מעל ה-MCP tools) | ⏳ מתוכנן | Phase 5.5.b/5.5.c — ה-webhook עצמו (`/api/whatsapp/webhook`, אימות `X-Hub-Signature-256`, דדופליקציה, היסטוריה בצד שרת) והקמת Meta/סודות. תשתית הקישור כבר קיימת (שורה מעל) |
