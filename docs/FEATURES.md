...
| App Check | ⚠️ מוגדר, טרם נאכף | Phase 4.4 — `src/lib/firebase/appCheck.ts` (reCAPTCHA **Enterprise**) + debug mode ל-dev/CI. המפתח נוצר, נרשם ומולא ב-`apphosting.yaml` (2026-08-29). נשאר: rollout → אימות verified requests → **Enforce** על Firestore/Storage (`docs/DEPLOYMENT.md`). עד ה-Enforce הטוקן נשלח אך לא נאכף, ו-threat #4 ב-`docs/SECURITY.md` פתוח. ראו `docs/DECISIONS.md` #26, #27, #28 |
