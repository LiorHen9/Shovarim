...
| בוט WhatsApp (שיחה מעל ה-MCP tools) | 🟡 קוד הושלם, הקמה נשארה | Phase 5.5.b — `POST/GET /api/whatsapp/webhook` (אימות `X-Hub-Signature-256` על הגוף הגולמי, דדופליקציה ב-`channelMessages`, `handleInboundChannelMessage` ניטרלי לספק, היסטוריה ב-`chatSessions`, rate limit על turns) + `npm run whatsapp:sim` לאימות בלי Meta. **חסר**: Meta app, מספר וסודות (5.5.c) — עד אז ה-endpoint מחזיר 503. ראו `docs/DECISIONS.md` #30 |
