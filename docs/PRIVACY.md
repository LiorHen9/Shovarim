...
| `phone` (E.164) | `listInviteCodes` | **לגסי בלבד מ-ADR #38** — הזמנות חדשות נכתבות עם `phone: null`. במסמכים ישנים: מזהה ישיר ו**מספר של צד שלישי**, שהוזן על ידי בעל הרשימה על מישהו אחר שאולי אינו משתמש במערכת כלל. ראו "מספרים של לא-משתמשים" למטה. נוסף ב-ADR #37 |
| `phone` (E.164) | `cardLists/{listId}/members` | **חשיפה חדשה כלפי בעל הרשימה** (ADR #38): מספר הווטסאפ של החבר מוצג לבעלים ב-`ShareListDialog` לצד המייל. מה שמצמצם: נשמר רק ברגע שהמוזמן עצמו מאשר במפורש את ההצטרפות, ורק אם כבר קישר מספר; **לא** נשלף מחדש מ-`channelLinks` בזמן תצוגה, כך שניתוק המספר לא חושף אותו שוב. נכלל בייצוא ובמחיקה כמו כל מסמך `members` |
...
**מספרים של לא-משתמשים (ADR #37, סגור מ-ADR #38)**: `listInviteCodes.phone` היה הפעם הראשונה שהמערכת שומרת PII של אדם ש**אינו** משתמש בה ולא נתן הסכמה — בעל רשימה הקליד מספר של מישהו אחר. **ADR #38 הסיר את הקלט הזה לגמרי**: השיתוף לא שואל למספר, ולכן המערכת כבר לא צוברת מספרים של לא-משתמשים. מה שנשאר הוא זנב של מסמכים ישנים, שכולם יפוגו תוך 14 יום מהמעבר. עד אז, מה שמצמצם: המספר נשמר רק כחלק מהזמנה פעילה, הבעלים יכול לבטל אותה, והיא נמחקת עם חשבון הבעלים. מה שעדיין **לא** מוסדר: (א) אין TTL policy בפועל שמוחק את המסמך אחרי הפקיעה (אותה חובה פתוחה כמו `channelLinkCodes`, ראו למטה) — כלומר מספר של אדם שלא הצטרף עשוי להישאר מאוחסן אחרי שההזמנה כבר חסרת תוקף; (ב) לאותו אדם אין דרך לבקש מחיקה או לדעת שהמספר שלו נשמר, כי אין לו חשבון שדרכו לפנות. שתי הנקודות דורשות התייחסות ב-Privacy Policy לצד סעיף ה-Meta למטה.
