...
| `clubCardId` | `clubMemberships` | הצהרה עצמית על כרטיסי מועדון שברשות המשתמש (ADR #61, Phase 10.1) — **נתוני שיוך/העדפה**, כלומר profiling data תחת GDPR, ולפעמים גם רמז לסטטוס אישי (למשל מועדון המיועד למשרתי מילואים). לא מאומת מול המועדון ואינו זכאות. הקטלוג עצמו (`clubs`/`clubCards`) משותף ואינו PII. נכלל בייצוא (עם שמות בטקסט מלא) ובמחיקת חשבון דרך `ownerId`. **אין נמען צד-שלישי חדש** — שלב 10.1 אינו מבצע אף בקשת HTTP יוצאת, וגם לוגואי המועדונים מוגשים כקבצים מקומיים מ-`public/clubs/` ולא כ-hotlink ל-CDN של המועדון, בדיוק כדי שהמשפט הזה יישאר נכון (ADR #61 החלטה 7), ולכן `PRIVACY_POLICY_VERSION` לא עלה. שכבת שליפת ההטבות (10.2) תצטרך בדיקה מחודשת של הסעיף הזה |
