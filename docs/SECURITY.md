...
- **מחיקה מתוזמנת היא Server Action רגיל** (`adminScheduleUserDeletionAction`/`adminCancelUserDeletionAction`, `src/actions/adminDeletion.ts`) — `requireAdmin()` בכניסה, אותו `deletionRequestedAt` ואותו grace-period sweep כמו מחיקה עצמית (Phase 4.2), בלי מנגנון מקביל.
- **מחיקה מיידית *אינה* Server Action** — היא Cloud Function `onCall` (`functions/src/adminActions.ts`, `adminDeleteUserNow`), כי `functions/tsconfig.json`'s `rootDir` חוסם import של `deleteUserAccount()` מתוך `src/actions/` (ADR #24). callable הוא endpoint ציבורי בפני עצמו — ה-UI (client component) קורא לו ישירות עם `httpsCallable`, ולכן הבדיקה "האם הקורא אדמין" **חייבת** לרוץ בתוך הפונקציה עצמה (`request.auth?.uid` מול `adminRoles/{uid}`), לא רק ב-UI. לוגיקת ההרשאה מופרדת מה-wrapper של `onCall` (`adminDeleteUserNowHandler`) בדיוק כדי שאפשר יהיה לבדוק אותה ישירות בלי transport אמיתי.
- **`enforceAppCheck: true` ברמת הפונקציה**: זה *לא* אותו מנגנון כמו ה-Console-level "Enforce" שמתועד ב-`docs/DEPLOYMENT.md` עבור Firestore/Storage (זה חל רק על שני אלה, לא על Cloud Functions) — כאן ה-SDK של הפונקציה עצמה מוודא טוקן App Check תקין ומחזיר 401 אוטומטית אם הוא חסר/לא תקין, לפני שה-handler בכלל רץ.
- **הגנה עצמית**: גם ב-`adminDeleteUserNow` (uid של הקורא) וגם ב-`scheduleUserDeletion` (uid) — אדמין לא יכול למחוק/לתזמן מחיקה לעצמו, אותה סיבת lockout כמו ב-Phase 9.3.
- **`adminAuditLog`**: `delete_scheduled`/`delete_cancelled`/`delete_immediate` נכתבים לפני המוטציה. עבור מחיקה מיידית זה מלבד (לא במקום) רשומת `auditLog` הפנימית שכבר קיימת בתוך `deleteUserAccount()`.
