...
### הזמנת רשימה לפי מספר טלפון (ADR #37, issue #58)
...
- **12 תווי base32 ולא 8** (בשונה מקוד הקישור): ה-TTL הוא 14 יום ולא 10 דקות, כלומר חלון ניחוש גדול בסדרי גודל. 32^12 (~10^18) משמר את יחס הבטיחות. `crypto.randomInt`, לא `Math.random`.
...
- **הבחנה שלא נחשפת**: "המספר לא מקושר לאף חשבון" ו"המספר מקושר לחשבון אחר" מוצגים כאותה חסימה מעשית, כדי לא ליצור oracle ל"האם המספר הזה רשום" — אותו שיקול שסוגר את `channelLinks` לקריאה.
- **דחייה לא דורשת קישור ערוץ**: סירוב אינו טענת זהות, ומי שקיבל לינק בטעות חייב יכולת לסגור אותו. אישור — כן.
