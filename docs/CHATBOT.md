...
| `scripts/run-chat-scenario.ts` | `npm run chat:scenario` — הרצת מקרה בדיקה מתועד (`docs/chatbot-test-cases/*.json`) מול uid סינתטי חדש |
...
| כפתור קישור בתשובה | אין — המשתמש כבר באתר | כפתור "לאזור האישי" אחרי פעולה משנת-מצב (issue #62) |

### כפתור "לאזור האישי" בתשובות (issue #62, ADR #60)

תשובה שמסכמת **פעולה שהבוט ביצע** יוצאת עם כפתור קישור ל-`/dashboard`; תשובה לשאלת קריאה בלבד ("מה היתרה שלי?") יוצאת בלי. הכפתור הוא הודעת `interactive`/`cta_url` של Meta — מותרת בתוך חלון 24 השעות ככל תשובה אחרת, בלי template.

ההחלטה מתקבלת **בקוד ולא ע"י המודל**: `channelChat.ts` אוסף את שמות הכלים שהסבב הריץ דרך `onToolCall` ומשווה מול `MUTATING_TOOL_NAMES` ב-`src/lib/mcp/toolEffects.ts`. אין הוראה על כך ב-system prompt, וה-`cta` הוא שדה נפרד ב-`ChannelReply` שלא נכנס לטקסט ולא נשמר ב-`chatSessions` — כלומר אפס טוקנים, גם בסבב הזה וגם בבאים.

> ⚠️ **כלי MCP חדש שכותב משהו חייב להתווסף ל-`MUTATING_TOOL_NAMES`.** אין כשל רועש אם שוכחים — התשובות של אותו כלי פשוט לא יקבלו כפתור. יש הערה על כך גם בגוף `createMcpServer`.

תשובה ארוכה מ-1024 תווים נשלחת כטקסט רגיל בלי כפתור: גוף של הודעה אינטראקטיבית מוגבל ל-1024 מול 4096 בטקסט, והכפתור לא שווה חיתוך של סוף התשובה.
